...
css_replacements = {
    ...
    ".cover-details p {": ".cover-details p {\n            text-align: right;\n            margin: 5px 0;\n        }",
    ".toc {": ".toc {\n            background-color: var(--bg-card);\n            padding: 30px;\n            border-radius: 8px;\n            margin-bottom: 50px;\n            box-shadow: 0 4px 10px rgba(0, 0, 0, 0.05);\n            border: 1px solid var(--border);\n        }"
}

# The previous agent's CSS might already have some of these, let's just do a regex substitution for safety
content = re.sub(r'\.cover-details\s*\{[^}]*\}', 
    ".cover-details {\n            margin-top: auto;\n            width: 100%;\n            padding-bottom: 30px;\n        }", content)

content = re.sub(r'\.cover-details\s*p\s*\{[^}]*\}', 
    ".cover-details p {\n            font-size: 1.1rem;\n            color: var(--text-light);\n            text-align: right;\n            margin: 5px 0;\n        }", content)
if ".cover-details p {" not in content:
    content = content.replace("</style>", "        .cover-details p {\n            font-size: 1.1rem;\n            color: var(--text-light);\n            text-align: right;\n            margin: 5px 0;\n        }\n    </style>")

content = re.sub(r'\.toc\s*\{[^}]*\}', 
    ".toc {\n            background-color: var(--bg-card);\n            padding: 30px;\n            border-radius: 8px;\n            margin-bottom: 50px;\n            box-shadow: 0 4px 10px rgba(0, 0, 0, 0.05);\n            border: 1px solid var(--border);\n        }", content)

# Now write the correct body exactly like the PDF
new_body = """<body>
...
<div class="container">
    ...
    <div class="highlight-box" style="background-color: transparent; border-left: 4px solid var(--secondary); padding: 15px 20px; margin: 20px 0;">
        <p style="margin: 0;">A segurança deste portal é absoluta por design. Como o projeto <strong>não possui banco de dados aberto na web</strong>, não há porta de entrada para que hackers roubem ou sequestrem informações da Contabilidade Camilo.</p>
    </div>
    <ul>
    ...
    <ul>
        ...
        <li><strong>Calculadora Salário Líquido</strong> <span style="color: var(--accent);">[Bônus PandaRoSan]</span> — Ferramenta complexa entregue integralmente como bônus não-orçado para maximizar exponencialmente a tração do escritório.</li>
    </ul>
    ...
    <div class="highlight-box" style="background-color: transparent; border-left: 4px solid var(--secondary); padding: 15px 20px; margin: 20px 0;">
        <p style="margin: 0;">Desenvolvemos este painel para ser uma usina de atração de leads via SEO orgânico. O preenchimento correto é vital para posicionar os artigos na primeira página do Google.</p>
    </div>
    <ul>
        ...
        <li><strong>Hierarquia Semântica (SEO):</strong> O H1 é injetado pelo sistema (o Título). No corpo, utilize <strong>H2</strong> para Tópicos Principais e <strong>H3</strong> para Subtópicos internos. Essa "escada" é vital para os robôs de busca.</li>
    ...
    <p>O grande diferencial da engenharia PandaRoSan é focar na eficiência global da empresa (redução de OpEx).</p>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>Armazenamento</td>
                <td>Uso ínfimo (0,001 GB).<br><strong>Converter em Alias ou Grupo.</strong></td>
            </tr>
            <tr>
                <td><code>financeiro@contabilidadecamilo.com.br</code></td>
                <td>Administrativo</td>
                <td>Uso baixo (0,39 GB).<br><strong>Converter em Alias ou Grupo.</strong></td>
            </tr>
            <tr>
                <td><code>fiscal2@contabilidadecamilo.com.br</code></td>
                <td>Setor Fiscal</td>
                <td>Uso baixo (0,08 GB).<br><strong>Converter em Alias Compartilhado.</strong></td>
            </tr>
            <tr>
    ...
    <div class="highlight-box" style="background-color: transparent; border-left: 4px solid var(--secondary); padding: 15px 20px; margin: 20px 0;">
        <p style="margin-bottom: 10px;"><strong>Plano Tático de Redução de Custos:</strong></p>
        <p style="margin-bottom: 10px;">A diretriz para os próximos meses visa a <strong>conversão das caixas departamentais</strong> (como <code>arquivos@</code>, <code>financeiro@</code>) em Aliases (apelidos) ou Grupos Colaborativos sem custo de licença no Workspace.</p>
        <p style="margin: 0;">Em paralelo, faremos a orquestração do <strong>Cloudflare Email Routing</strong>, permitindo redirecionar e-mails nativamente recebidos pelo domínio secundário diretamente para as caixas-mãe sem demandar licenças atreladas. O impacto estimado atinge a mitigação e o <strong>corte direto no desperdício de 3 a 5 licenças em moeda forte (Dólar) mensalmente</strong>, mantendo 100% da identidade institucional para o público e sem ferir a rotina da equipe de colaboradores.</p>
    </div>
    ...
    <hr style="margin: 60px 0 30px; border: none; border-top: 1px solid var(--border);">
    <div style="text-align: center; color: var(--text-main); font-weight: 600;">
        PandaRoSan IT Solutions © 2026
    </div>
    <div style="text-align: center; color: var(--text-light); font-style: italic; margin-top: 10px;">
        Inovação, Segurança e Engenharia de Vendas para o Setor Contábil.
    </div>
